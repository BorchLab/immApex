...
.calculateGrantham <- function(vec1, vec2) {
  valid_pairs <- (vec1 %in% amino_acids) & (vec2 %in% amino_acids)
  
  if (!all(valid_pairs)) {
    invalid_indices <- which(!valid_pairs)
    for(i in invalid_indices) {
      warning(paste("Skipping invalid amino acid pair:", vec1[i], vec2[i], "at position", i))
    }
    # Filter out the invalid pairs before calculation
    vec1 <- vec1[valid_pairs]
    vec2 <- vec2[valid_pairs]
  }
  
  # If no valid pairs remain after filtering, return 0
  if (length(vec1) == 0) {
    return(0)
  }
  sum(grantham_matrix[cbind(vec1, vec2)])
}
...
# Amino acid codes
mm.aa <- c("S", "R", "L", "P", "T", "A", "V", "G", "I", "F", "Y", "C", "H", "Q", "N", "K", "D", "E", "M", "W")

# Create the Grantham distance matrix
grantham_matrix <- matrix(grantham_data, nrow = 20, ncol = 20, byrow = TRUE,
                          dimnames = list(mm.aa, mm.aa))